...
// Waiting users for anonymous match (TEMP – OK for now)
const waitingQueue = [];
...
const initializeSocket = (server) => {
  ...
  io.use(async (socket, next) => {
    try {
      const token = socket.handshake.auth?.token;
      if (!token) {
        console.log("❌ Socket without token");
      ...
      try {
        ...
        if (matchedUser) {
          ...
          if (otherSocket) {
            ...
            console.log("🟢 Other user joined room:", matchedUser.userId, roomId);
          }
